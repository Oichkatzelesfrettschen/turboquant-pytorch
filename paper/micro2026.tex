...
\title{Architecture-Aware KV Cache Quantization:\\Distribution Normalization with Structured Rotation Dispatch}
...
\begin{abstract}
KV cache memory dominates long-context LLM inference, scaling
linearly with context length. We present an architecture-aware
quantization pipeline with three contributions.

First, we identify that per-channel variance ratios up to $423\times$ in
real KV caches render Gaussian-optimal codebooks suboptimal.
Normalize-Shift-Normalize pre-processing reduces this ratio to
$1.01\times$, improving attention cosine similarity by 136 basis points
and top-1 match by 25 percentage points on Qwen2.5-3B and
Mistral-7B---the dominant quality factor in our ablation.

Second, we implement architecture-aware execution: materialized
Walsh-Hadamard rotation via cuBLAS with automatic TF32 dispatch
($3\times$ throughput on Ampere+) at $64\times$ fewer parameters than dense
Haar rotation, and bit-packed QJL signs reducing per-score
attention bandwidth from 256\,B to 34\,B. Five-tier GPU dispatch
(Hopper through generic) auto-selects precision modes.

Third, we ground the rotation design space in Cayley-Dickson
algebra, proving isometric guarantees for structured block
rotations (55 statements across 30 proof groups, SMT-verified). Ablation reveals a
surprising dichotomy: distribution normalization determines
quality ($>$99\% of gain) while architecture-aware dispatch
...
Real inference validation on Qwen2.5-3B (36 layers, 512 tokens)
yields average key cosine of 0.873 (2-bit), 0.961 (3-bit),
and 0.989 (4-bit). Per-layer analysis reveals that the worst layer
(L22, cosine 0.954 at 3-bit) has the \emph{lowest} key variance
(2.64) and kurtosis (3.35), suggesting that low-dynamic-range keys
are proportionally more affected by quantization noise. Paradoxically,
layers with the highest channel variance ratio (up to $17{,}431\times$,
L0) achieve the best cosine after NSN normalization.
This confirms that NSN is most beneficial precisely where the
pre-processing challenge is greatest.
...
$S=512$, 36 layers, 2 KV heads via GQA). Measured on RTX 4070 Ti.}
\label{tab:memory}
\begin{tabular}{lrrr}
\toprule
Component & Vanilla & Ours & Reduction \\
\midrule
MSE indices (int8/coord) & 4.7\,MB & 4.7\,MB & --- \\
QJL signs (int8 vs packed) & 4.7\,MB & 0.6\,MB & $8\times$ \\
Residual norms (FP16) & 0.3\,MB & 0.3\,MB & --- \\
Vector norms (FP16) & 0.3\,MB & 0.3\,MB & --- \\
NSN metadata & --- & 0.04\,MB & (new) \\
\midrule
\textbf{Total compact} & \textbf{10.0\,MB} & \textbf{5.9\,MB} & $\mathbf{1.7\times}$ \\
FP16 baseline & 18.0\,MB & --- & --- \\
Compact ratio (vs FP16) & $1.80\times$ & $\mathbf{3.05\times}$ & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Free bits.}
With int8 index storage, the MSE quantization indices
occupy the same space whether encoding 4, 8, or 16 codebook levels.
In practice, upgrading from 3-bit to 4-bit (8 to 16 levels) improves
average cosine from 0.961 to 0.989 on Qwen2.5-3B---a $+2.8$\,pp
gain at zero additional storage cost. The ``bit-width'' parameter
thus controls codebook granularity, not storage, when indices are
stored as bytes rather than bit-packed.

\paragraph{MSE-cosine dichotomy.}
Lagrange-optimal per-layer allocation (minimizing total MSE) assigns
more bits to \emph{high}-variance layers, while cosine-optimal
allocation assigns more bits to \emph{low}-variance layers.
On Qwen2.5-3B (36 layers, 3.5-bit average budget):
Lagrange achieves $2.1\times$ lower MSE (9.50 vs.\ 19.67), but
greedy-cosine achieves $+1.6$\,pp better attention cosine
(0.976 vs.\ 0.974). The explanation:
$\cos \approx 1 - \text{MSE}/(2\sigma^2)$, so low-variance regions
are most \emph{cosine-sensitive} even though high-variance regions
dominate total MSE. This dichotomy means the allocation strategy
should match the downstream metric: MSE for reconstruction,
cosine for attention fidelity.
...
\subsection{E8 Lattice + CD Rotation Breakthrough}

Full matrix evaluation of 4 rotations $\times$ 3 quantizers on Qwen2.5-3B
reveals a striking synergy between CD8 rotation and E8 lattice quantization.

\begin{table}[t]
\centering
\caption{Rotation $\times$ quantizer matrix on Qwen2.5-3B (36 layers,
$d=128$, 401 tokens). All rows include NSN pre-processing.}
\label{tab:e8breakthrough}
\begin{tabular}{llccr}
\toprule
Rotation & Quantizer & Cosine & MSE & bits/dim \\
\midrule
WHT & Scalar (3b) & 0.958 & 0.276 & $\sim$3 \\
WHT & Scalar (4b) & 0.989 & 0.080 & $\sim$4 \\
WHT & E8 lattice & 0.989 & 0.078 & $\sim$1 \\
\textbf{CD8} & \textbf{E8 lattice} & \textbf{0.990} & \textbf{0.076} & $\mathbf{\sim 1}$ \\
\bottomrule
\end{tabular}
\end{table}

CD8+E8+NSN achieves 0.990 cosine at $\sim$1\,bit/dim, matching 4-bit
scalar (0.989) at $4\times$ lower bit rate. The structural explanation:
CD8 (octonion left-multiplication) rotates in 8D blocks; E8 quantizes
in 8D blocks. The block alignment means the rotation maps data into
the \emph{Voronoi regions} of E8, where the sphere packing
($G(\text{E8}) = 0.0717$, the densest in 8D by Viazovska~2016)
achieves near-optimal quantization distortion.

In real autoregressive generation, CD8+E8+NSN achieves 49.1\% exact
token match (vs.\ 33.0\% for 3-bit scalar and 69.6\% for 4-bit scalar),
confirming the rate-distortion improvement translates to generation quality.
...
We validate compressed KV caches in real autoregressive generation
by reconstructing the cache from quantized indices and continuing
decoding. On Qwen2.5-3B at 3-bit, compressed outputs achieve
23--38\% exact token match with baseline (expected due to
autoregressive error accumulation) while maintaining coherent,
factually correct text. Per-layer key cosine averages 0.961
across all 36 layers; the worst layer (L22) still achieves 0.954.
...
(31 proof groups, 58 statements Z3-proven). The key finding is
\emph{structural synergy}: CD8 rotation (octonion block) combined
with E8 lattice quantization achieves 0.990 cosine at $\sim$1\,bit/dim,
matching 4-bit scalar quality at $4\times$ lower bit rate. This
arises because both operators share 8D block structure aligned with
the densest sphere packing. Lagrange-optimal bit allocation reveals
a dual metric: MSE-optimal and cosine-optimal strategies are opposite,
requiring metric-aware allocation. The full pipeline compresses at
43K~vectors/sec on RTX~4070~Ti and produces coherent autoregressive
generation from compressed caches (49\% token match at 1\,bit/dim).
